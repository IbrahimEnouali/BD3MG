...
class APAR3MG_Master_worker(mp.Process):
    ...
    def run(self):
        ...
        for c in range(19): #self.num_workers-1

            self.sending_update(list(self.connections[c])[0], c, self.init[c], self.p3) # self.sending_update(self.connections[c][0], c, self.init[c], self.p3)
        print('initialized')
