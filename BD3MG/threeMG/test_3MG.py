...
MMMG = MajorizeMinimizeMemoryGradient3D(y, h, Hty, H1, eta, tau, lambda_, delta, xmin, xmax, phi, x0, I, Nx, Ny, Nz, NbIt, cores, Timemax)
MMMG.optimize()
...
f = open("Times_MMMG.txt","w")
f.write( str(Times) )
f.close()

f = open("Crits_MMMG.txt","w")
f.write( str(Crits) )
f.close()

f = open("SNR_MMMG.txt","w")
f.write( str(SNR) )
f.close()