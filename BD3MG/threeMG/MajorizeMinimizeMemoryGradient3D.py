import time
import numpy as np
from threeMG.Critere3D import Critere3D
from threeMG.Voperator3D import *
...
class MajorizeMinimizeMemoryGradient3D:
    ...
    def optimize(self):
        ...
        for k in range(self.NbIt):
            ...
            if k == 0:
                ...
                s = np.sum(Grad**2)/B
                #print(np.linalg.norm(B.flatten(),1))
                #print(s)
    ...
    def majorante2(self, x, d1, d2, Vvd1, Vhd1, Vtd1, Vvd2, Vhd2, Vtd2, Hd1, Hd2, phi, eta, lambda_, kappa, delta, xmin, xmax):
        ...
        d1tVtWVd1 = self.lambda_*np.sum(Vvd1.copy()*(wXY_Vx.copy()*Vvd1.copy())+Vhd1.copy()*(wXY_Vx.copy()*Vhd1.copy()))+ self.kappa*np.sum(Vtd1.copy()*(wZ_Vx.copy()*Vtd1.copy()))
        d1tVtWVd2 = self.lambda_*np.sum(Vvd1.flatten()*(wXY_Vx.flatten()*Vvd2.flatten())+Vhd1.flatten()*(wXY_Vx.flatten()*Vhd2.flatten()))+ self.kappa*np.sum(Vtd1.flatten()*(wZ_Vx.flatten()*Vtd2.flatten()))
        d2tVtWVd2 = self.lambda_*np.sum(Vvd2.flatten()*(wXY_Vx.flatten()*Vvd2.flatten())+Vhd2.flatten()*(wXY_Vx.flatten()*Vhd2.flatten()))+ self.kappa*np.sum(Vtd2.flatten()*(wZ_Vx.flatten()*Vtd2.flatten()))
